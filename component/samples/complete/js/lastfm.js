...
function _lastfm() {
	...
	this.post = (method, token, metadb) => {
		...
		case 'auth.getToken':
			this.update_sk('');
			api_sig = md5('api_key' + this.api_key.value + 'method' + method + this.secret);
			data = 'format=json&method=' + method + '&api_key=' + this.api_key.value + '&api_sig=' + api_sig;
			break;
		case 'auth.getSession':
			api_sig = md5('api_key' + this.api_key.value + 'method' + method + 'token' + token + this.secret);
			data = 'format=json&method=' + method + '&api_key=' + this.api_key.value + '&api_sig=' + api_sig + '&token=' + token;
			break;
		case 'track.love':
		case 'track.unlove':
			switch (true) {
			...
			api_sig = md5('api_key' + this.api_key.value + 'artist' + artist + 'method' + method + 'sk' + this.sk + 'track' + track + this.secret);
			// can't use format=json because Last.fm API is broken for this method
			data = 'method=' + method + '&api_key=' + this.api_key.value + '&api_sig=' + api_sig + '&sk=' + this.sk + '&artist=' + encodeURIComponent(artist) + '&track=' + encodeURIComponent(track);
			break;
		default:
			return;
		}
	...
	this.done = (method, metadb) => {
		...
		case 'auth.getToken':
			data = _jsonParse(this.xmlhttp.responseText);
			if (data.token) {
				_run('https://last.fm/api/auth/?api_key=' + this.api_key.value + '&token=' + data.token);
	...
	this.api_key = new _p('2K3.LASTFM.APIKEY', ''); // 1f078d9e59cb34909f7ed56d7fc64aba
