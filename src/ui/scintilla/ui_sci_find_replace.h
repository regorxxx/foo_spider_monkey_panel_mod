...
public:
                                              ...
                                              HWND hWndParent = nullptr)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                lastState_.findText = qwr::ToU8(findText);
                lastState_.replaceText = qwr::ToU8(replaceText);
    ...
    void OnReplaceAll(LPCTSTR lpszFind, LPCTSTR lpszReplace, BOOL bMatchCase, BOOL bWholeWord)
    {
        ...
        {
            pBase->TextNotFound(qwr::ToWide(lastState_.findText).c_str());
            return;
        }
    ...
    bool FindTextSimple(FindReplaceState::Direction direction)
    {
        ...
        {
            BaseClass::TextNotFound(qwr::ToWide(lastState_.findText).c_str());
            return false;
        }
        else
        {
...
private:
    ...
    bool UpdateFindReplaceState()
    {
        ...
        if (pFindReplaceDialog)
        {
            ...
            lastState_.findText = qwr::ToU8(std::wstring_view{ findCStr ? findCStr : L"" });
            if (!isFindOnlyDialog_)
            {
                const auto replaceCStr = pFindReplaceDialog->GetReplaceString();
                lastState_.replaceText = qwr::ToU8(std::wstring_view{ replaceCStr ? replaceCStr : L"" });
            }
