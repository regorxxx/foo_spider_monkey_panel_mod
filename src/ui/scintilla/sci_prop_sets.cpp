...
ScintillaPropsCfg props(guid::scintilla_props);
...
void ScintillaPropsCfg::get_data_raw(stream_writer* p_stream, abort_callback& p_abort)
{
    try
    {
        ...
        {
            p_stream->write_string(prop.key, p_abort);
            p_stream->write_string(prop.val, p_abort);
        }
    }
    catch (...)
    {
...
void ScintillaPropsCfg::set_data_raw(stream_reader* p_stream, t_size p_sizehint, abort_callback& p_abort)
{
    ...
    try
    {
        ...
        {
            key = p_stream->read_string(p_abort).get_ptr();
            val = p_stream->read_string(p_abort).get_ptr();
            data_map[key] = val;
        }
    }
    catch (...)
...
void ScintillaPropsCfg::export_to_file(fb2k::stringRef path)
{
    std::string content = "# Generated by " SMP_NAME "\r\n";
    ...
    TextFile(wpath).write(content);
}

void ScintillaPropsCfg::import_from_file(fb2k::stringRef path)
{
    const auto native = filesystem::g_get_native_path(path->c_str());
    const auto wpath = qwr::ToWide(native);
    const auto text = TextFile(wpath).read();
...
bool ScintillaPropsCfg::StriCmpAscii::operator()(const std::string& a, const std::string& b) const
{
